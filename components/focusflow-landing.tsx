...
export function FocusFlowLanding() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-background text-foreground">
      ...
      <main id="top">
        <section className="relative isolate mx-auto max-w-6xl overflow-hidden px-5 pb-28 pt-20 text-center lg:px-8 lg:pb-36 lg:pt-32"><div className="pointer-events-none absolute left-1/2 top-[-8rem] -z-10 h-[34rem] w-[52rem] -translate-x-1/2 rounded-full bg-primary/14 blur-3xl" /><div className="pointer-events-none absolute left-1/2 top-16 -z-10 h-72 w-72 -translate-x-1/2 rounded-full bg-violet-400/10 blur-[100px]" /><div className="mx-auto mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1.5 text-xs font-medium text-muted-foreground shadow-sm"><Sparkles className="size-3.5 text-primary" /> Built for the way you actually study</div><h1 className="mx-auto max-w-5xl text-balance font-display text-5xl font-extrabold leading-[0.98] tracking-[-0.055em] sm:tracking-[-0.08em] lg:tracking-[-0.095em] sm:text-7xl lg:text-[5.5rem]">Your syllabus, turned into a plan <span className="text-primary">you&apos;ll actually follow.</span></h1><p className="mx-auto mt-7 max-w-xl text-pretty text-sm leading-7 text-muted-foreground sm:text-base">FocusFlow turns every deadline, reading, and exam into a calm daily rhythm that adapts with you — not against you.</p><div className="mt-9 flex justify-center"><a id="start" href="#product" className="group inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3.5 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/25 transition duration-200 hover:scale-[1.03] hover:bg-primary/90 hover:shadow-xl hover:shadow-primary/30 active:scale-[0.98]">Get Started Free <ArrowRight className="size-4 transition-transform group-hover:translate-x-0.5" /></a></div></section>
